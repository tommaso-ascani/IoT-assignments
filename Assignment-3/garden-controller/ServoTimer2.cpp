...
extern bool stopIr = false;
bool firstTimeInWork = true;
bool goUp = true;
int degreeAngle = 0;
...
void ServoTimer2::moveForward()
{
  velocity = map(this->velocity,1,5,6,2);
...
void ServoTimer2::moveBackward()
{
  this->write(750);
}

bool ServoTimer2::work()
{
  if(firstTimeInWork){
    startMillis = millis();
    firstTimeInWork = false;
    goUp = true;
  }
  if(millis() <= startMillis + WORK_TIME){
    int angle = map(degreeAngle, 0, 180, 750, 2250);
    int velocity = map(this->velocity, 1, 4, 0, 3);
    this->write(angle);
    if (goUp){
      switch(velocity) {
        case 0 : {
  ...
  firstTimeInWork = true;
  return false;
}
  
//  long int startMillis = millis();
//  while(millis() <= startMillis + WORK_TIME){
//    this->moveForward();
//    if(stopIr == true){
//      break;
//    }
...
void ServoTimer2::setVelocity(int vel) {
  this->velocity = vel;
}
